...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        const lineItems = products.map(product => {
            const amount = Math.round(product.price * 100) // stripe wants price to be sent in format of cents
...
export const checkoutSuccess = async (req, res) => {
    try {
        ...
        console.error("Error processing successul checkout:", error);
        res.status(500).json({ success: false, message: "Error processing successul checkout:", error: error.message });
    }
}
